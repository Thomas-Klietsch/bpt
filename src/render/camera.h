...
namespace Render
{
	...
	public:
		...
		Camera(
			...
			: position( position ), image_width( config.image_width ), image_height( config.image_height ), n_samples( config.n_samples )
		{
			...
			// If view direction and World Up is colinear (or close to it), change World up axis.
		...
		{
			auto [e1, e2] = random[ sample % n_samples ];
